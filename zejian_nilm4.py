...
def get_activation(input_data, window1=100):
    ...
    # print('S3')
    #导数也要smooth
    gauss_d = gauss_d.rolling(window=window1, win_type='gaussian', center=True).mean(std=10)
    ...
    if len(zeros_index) == 0:
        ...
        cc = []
        return zeros_index, zeros_index, cc
    ...
    gauss_d2 = gauss_d2.rolling(window=10, win_type='gaussian', center=True).mean(std=10)
    # 滤掉小刺波
    ...
    pass
    return activation_start, activation_end, event_index

# 从active power得到的 event time之后，我们需要得到这个时间的其他数据，比如无功，电流，电压之类的
def get_others(input_data, event_index):
    activation_start=[]
    activation_end=[]

    # 小高斯平滑
    mains_gauss2 = input_data.rolling(window=20, win_type='gaussian', center=True).mean(std=10)
    # mains_gauss2 = input_data
    # mains_gauss2=mains.rolling(window=9, win_type='gaussian', center=True).mean(std=10) # good
    # 求导
    gauss_d2 = mains_gauss2.diff()
    # 滤掉小刺波
